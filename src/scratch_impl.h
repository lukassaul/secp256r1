...
#ifndef SECP256R1_SCRATCH_IMPL_H
#define SECP256R1_SCRATCH_IMPL_H

#include "util.h"
#include "scratch.h"

static secp256r1_scratch* secp256r1_scratch_create(const secp256r1_callback* error_callback, size_t size) {
    const size_t base_alloc = ROUND_TO_ALIGN(sizeof(secp256r1_scratch));
    void *alloc = checked_malloc(error_callback, base_alloc + size);
    secp256r1_scratch* ret = (secp256r1_scratch *)alloc;
    if (ret != NULL) {
...
static void secp256r1_scratch_destroy(const secp256r1_callback* error_callback, secp256r1_scratch* scratch) {
    if (scratch != NULL) {
        if (secp256r1_memcmp_var(scratch->magic, "scratch", 8) != 0) {
            secp256r1_callback_call(error_callback, "invalid scratch space");
            return;
        }
...
static void secp256r1_scratch_apply_checkpoint(const secp256r1_callback* error_callback, secp256r1_scratch* scratch, size_t checkpoint) {
    ...
    if (checkpoint > scratch->alloc_size) {
        secp256r1_callback_call(error_callback, "invalid checkpoint");
        return;
    }
    scratch->alloc_size = checkpoint;
}

static size_t secp256r1_scratch_max_allocation(const secp256r1_callback* error_callback, const secp256r1_scratch* scratch, size_t objects) {
    if (secp256r1_memcmp_var(scratch->magic, "scratch", 8) != 0) {
        secp256r1_callback_call(error_callback, "invalid scratch space");
        return 0;
    }
...
static void *secp256r1_scratch_alloc(const secp256r1_callback* error_callback, secp256r1_scratch* scratch, size_t size) {
    void *ret;
    size_t rounded_size;
