...
#ifndef SECP256R1_ECKEY_IMPL_H
#define SECP256R1_ECKEY_IMPL_H
...
static int secp256r1_eckey_pubkey_parse(secp256r1_ge *elem, const unsigned char *pub, size_t size) {
    if (size == 33 && (pub[0] == SECP256R1_TAG_PUBKEY_EVEN || pub[0] == SECP256R1_TAG_PUBKEY_ODD)) {
        secp256r1_fe x;
        return secp256r1_fe_set_b32(&x, pub+1) && secp256r1_ge_set_xo_var(elem, &x, pub[0] == SECP256R1_TAG_PUBKEY_ODD);
    } else if (size == 65 && (pub[0] == SECP256R1_TAG_PUBKEY_UNCOMPRESSED || pub[0] == SECP256R1_TAG_PUBKEY_HYBRID_EVEN || pub[0] == SECP256R1_TAG_PUBKEY_HYBRID_ODD)) {
        secp256r1_fe x, y;
        if (!secp256r1_fe_set_b32(&x, pub+1) || !secp256r1_fe_set_b32(&y, pub+33)) {
            return 0;
        }
        secp256r1_ge_set_xy(elem, &x, &y);
        if ((pub[0] == SECP256R1_TAG_PUBKEY_HYBRID_EVEN || pub[0] == SECP256R1_TAG_PUBKEY_HYBRID_ODD) &&
            secp256r1_fe_is_odd(&y) != (pub[0] == SECP256R1_TAG_PUBKEY_HYBRID_ODD)) {
            return 0;
        }
        return secp256r1_ge_is_valid_var(elem);
    } else {
        return 0;
    }
}
...
static int secp256r1_eckey_pubkey_serialize(secp256r1_ge *elem, unsigned char *pub, size_t *size, int compressed) {
    ...
    secp256r1_fe_normalize_var(&elem->x);
    secp256r1_fe_normalize_var(&elem->y);
    secp256r1_fe_get_b32(&pub[1], &elem->x);
    if (compressed) {
        *size = 33;
        pub[0] = secp256r1_fe_is_odd(&elem->y) ? SECP256R1_TAG_PUBKEY_ODD : SECP256R1_TAG_PUBKEY_EVEN;
    } else {
        *size = 65;
        pub[0] = SECP256R1_TAG_PUBKEY_UNCOMPRESSED;
...
static int secp256r1_eckey_privkey_tweak_add(secp256r1_scalar *key, const secp256r1_scalar *tweak) {
    ...
    return !secp256r1_scalar_is_zero(key);
}
...
static int secp256r1_eckey_privkey_tweak_mul(secp256r1_scalar *key, const secp256r1_scalar *tweak) {
    int ret;
    ret = !secp256r1_scalar_is_zero(tweak);
