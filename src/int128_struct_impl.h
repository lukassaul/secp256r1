#ifndef SECP256R1_INT128_STRUCT_IMPL_H
#define SECP256R1_INT128_STRUCT_IMPL_H

#include "int128.h"

#if defined(_MSC_VER) && (defined(_M_X64) || defined(_M_ARM64)) /* MSVC */
#    include <intrin.h>
#    if defined(_M_ARM64) || defined(SECP256R1_MSVC_MULH_TEST_OVERRIDE)
/* On ARM64 MSVC, use __(u)mulh for the upper half of 64x64 multiplications.
   (Define SECP256R1_MSVC_MULH_TEST_OVERRIDE to test this code path on X64,
   which supports both __(u)mulh and _umul128.) */
#        if defined(SECP256R1_MSVC_MULH_TEST_OVERRIDE)
#            pragma message(__FILE__ ": SECP256R1_MSVC_MULH_TEST_OVERRIDE is defined, forcing use of __(u)mulh.")
#        endif
static SECP256R1_INLINE uint64_t secp256r1_umul128(uint64_t a, uint64_t b, uint64_t* hi) {
    *hi = __umulh(a, b);
    return a * b;
}

static SECP256R1_INLINE int64_t secp256r1_mul128(int64_t a, int64_t b, int64_t* hi) {
    *hi = __mulh(a, b);
    return (uint64_t)a * (uint64_t)b;
}
#    else
/* On x84_64 MSVC, use native _(u)mul128 for 64x64->128 multiplications. */
#        define secp256r1_umul128 _umul128
#        define secp256r1_mul128 _mul128
#    endif
#else
...
static SECP256R1_INLINE void secp256r1_i128_dissip_mul(secp256r1_int128 *r, int64_t a, int64_t b) {
   int64_t hi;
   uint64_t lo = (uint64_t)secp256r1_mul128(a, b, &hi);
   hi += r->lo < lo;
   /* Verify no overflow.
...
static SECP256R1_INLINE void secp256r1_i128_det(secp256r1_int128 *r, int64_t a, int64_t b, int64_t c, int64_t d) {
   secp256r1_i128_mul(r, a, d);
   secp256r1_i128_dissip_mul(r, b, c);
}
