...
#ifndef SECP256R1_MODULE_RECOVERY_BENCH_H
#define SECP256R1_MODULE_RECOVERY_BENCH_H
...
void bench_recover(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        int j;
        size_t pubkeylen = 33;
        secp256r1_ecdsa_recoverable_signature sig;
        CHECK(secp256r1_ecdsa_recoverable_signature_parse_compact(data->ctx, &sig, data->sig, i % 2));
        CHECK(secp256r1_ecdsa_recover(data->ctx, &pubkey, &sig, data->msg));
        CHECK(secp256r1_ec_pubkey_serialize(data->ctx, pubkeyc, &pubkeylen, &pubkey, SECP256R1_EC_COMPRESSED));
        for (j = 0; j < 32; j++) {
...
void run_recovery_bench(int iters, int argc, char** argv) {
    ...
    data.ctx = secp256r1_context_create(SECP256R1_CONTEXT_VERIFY);
...
#endif /* SECP256R1_MODULE_RECOVERY_BENCH_H */
